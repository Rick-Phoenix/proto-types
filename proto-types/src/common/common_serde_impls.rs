...
impl<'de> Deserialize<'de> for DayOfWeek {
		...
		D: Deserializer<'de>,
	{
		...
		impl Visitor<'_> for DayOfWeekVisitor {
			...
			fn expecting(&self, formatter: &mut core::fmt::Formatter) -> core::fmt::Result {
				formatter
					.write_str("a string representing a DayOfWeek enum variant (e.g., \"MONDAY\")")
			}
...
impl<'de> Deserialize<'de> for Month {
		...
		D: Deserializer<'de>,
	{
		...
		impl Visitor<'_> for MonthVisitor {
			...
			fn expecting(&self, formatter: &mut core::fmt::Formatter) -> core::fmt::Result {
				formatter
					.write_str("a string representing a Month enum variant (e.g., \"JANUARY\")")
			}
			...
			where
				E: de::Error,
			{
				Month::from_str_name(v)
					.ok_or_else(|| E::custom(format!("unknown Month variant: {v}")))
			}
		}
